    ...
    <AnimatePresence mode="wait">
      ...
      >
        {navItems.map((navItem: any, idx: number) => {
          const isCta = navItem.link === "/apply";
          return (
            <Link
              key={`link-${idx}`}
              href={navItem.link}
              className={cn(
                ...
                isCta
                  ? "rounded-full bg-gradient-to-r from-purple/80 to-blue-100/60 px-4 py-1.5 text-sm font-semibold text-white shadow-md transition hover:opacity-90"
